...
export default function NodeConfigPanel({
  ...
  return (
    <aside className="w-72 border-l border-green-500/20 bg-gray-950 p-3 space-y-3">
      ...
      {node.kind === 'choice' && (
        <div>
          <label className="text-xs text-gray-300">Opções (separe por quebra de linha)</label>
          <textarea
            rows={4}
      ...
      <div className="pt-2 border-t border-green-500/20">
        <button
          onClick={onDelete}
          className="w-full rounded-lg border border-red-500/30 text-red-300 text-sm py-2 hover:bg-red-500/10"
        >
          Remover nó
        </button>
      </div>
